...
export function getClients(limit = 200, offset = 0) {
  return apiFetch(`/clients?limit=${limit}&offset=${offset}&sortBy=createdAt&sortDirection=desc`);
}

export function getClientTickets(clientId, limit = 200, offset = 0) {
  return apiFetch(
    `/clients/${clientId}/tickets?limit=${limit}&offset=${offset}&sortBy=createdAt&sortDirection=desc`
  );
}

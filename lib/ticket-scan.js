...
// manual tipo stage_labels. `journeys` inclui o HISTÓRICO inteiro do
// cliente, inclusive jornadas já concluídas (`journeyCompleted: true`,
// confirmado ao vivo — um cliente antigo pode ter 2-3 jornadas encerradas
// listadas ali); só as que ainda não terminaram contam como "o cliente
// está nessa jornada agora", que é o que /jornada segue. Um cliente pode
// estar em mais de uma jornada ativa ao mesmo tempo (raro, mas visto ao
// vivo), daí devolver todas, sem repetir. `null` quando não está em
// nenhuma jornada ativa (não é um array vazio, pra distinguir de "sabemos
// que não tem" — ver processTicket em lib/ticket-notify.js, que preserva o
// valor salvo quando não vier nada).
export function extractJourneyNames(client) {
  ...
  const names = [
    ...new Set(journeys.filter((j) => j.journeyCompleted !== true).map((j) => j.journeyName).filter(Boolean)),
  ];

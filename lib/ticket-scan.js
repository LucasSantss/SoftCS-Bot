...
// Prioriza o que estiver salvo em stage_labels (manual, via clique no nome
// da coluna na aba Tickets, ou auto-gravado por processTicket() na primeira
// vez que o estágio aparece — ver lib/ticket-notify.js) sobre o nome vindo
// da própria API (denormalizedStage/stage), pra uma renomeação manual não
// ser sobrescrita depois. Só cai no nome genérico por ID se nenhum dos dois
// existir (ex: ticket sem nenhum ticket_state salvo ainda pra essa coluna).
// `stageLabels` é um mapa stage_id -> { label, position }.

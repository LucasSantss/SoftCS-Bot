...
// inscrição pessoal via /status) OU onde alguém segue uma das jornadas do
// cliente dono do ticket (chat_journeys, inscrição via /jornada) — os dois
// são ADITIVOS: um ticket pode notificar o chat do criador E o(s) chat(s)
// inscritos na jornada do cliente, sem duplicar se for o mesmo chat. Só cai
// no fallback de grupos/canais (nunca em chats pessoais — is_personal) se
// NENHUM dos dois achar alvo nenhum; senão qualquer ticket de um criador ou
// jornada sem mapeamento vazava pra todo mundo inscrito, sem filtro nenhum
// (bug real encontrado ao vivo). Cada item devolvido tem { chatId, threadId }
...
export async function getTargetChatIds(createdById, journeyNames) {
  ...
  if (journeyNames?.length) {
    const rows = await sql`
      select c.chat_id, c.thread_id
      from chat_journeys j
      join telegram_chats c on c.chat_id = j.chat_id
      where j.journey_name = any(${journeyNames}) and c.active = true
    `;
    for (const r of rows) targets.set(r.chat_id, { chatId: r.chat_id, threadId: r.thread_id });
  }

...
// sem duplicar se for o mesmo chat. SEM fallback: um chat/grupo cadastrado
// na aba Chats só recebe ticket de quem está especificamente vinculado a
// ele (por criador ou por jornada) — nunca por "sobra" de um criador sem
// mapeamento. Isso é de propósito (pedido explícito, depois de um vazamento
// real onde um ticket sem mapeamento acabava notificando todo mundo): se
// nada bater, `notifyTicketEvent()` não manda nada pra ninguém, em vez de
// broadcast pros grupos. Cada item devolvido tem { chatId, threadId } —
// threadId vem de telegram_chats.thread_id quando o chat é um Tópico
// específico dentro de um grupo (ver aba Chats).
...
export async function notifyTicketEvent({ kind, title, priority, publicId, clientName, stageId, createdById, journeyNames }) {
  ...
  if (targets.length === 0) {
    // Sem fallback de propósito (ver getTargetChatIds) — esse ticket não
    // notifica ninguém porque nem o criador nem a jornada do cliente batem
    // com nenhum chat/grupo cadastrado.
    console.warn(`Nenhum chat vinculado ao criador ${createdById ?? '(?)'} nem à(s) jornada(s) do cliente — ticket "${title ?? '(sem título)'}" não notificou ninguém.`);
    return;
  }

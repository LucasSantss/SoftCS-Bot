...
async function refreshAccessToken(refreshToken) {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('Credenciais OAuth da SoftCS não configuradas — cadastre em /admin.html');
  }
...
// O primeiro par access_token/refresh_token é criado pelo fluxo /api/oauth-start
// -> /api/oauth-callback (acionado a partir de /admin.html).
export async function getValidAccessToken() {
  const rows = await sql`select access_token, refresh_token, expires_at from softcs_oauth_tokens where id = 1`;
  const row = rows[0];

  if (!row) {
    throw new Error('Integração SoftCS ainda não autorizada — acesse /admin.html e clique em "Autorizar".');
  }
